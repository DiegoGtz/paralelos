 ...
 *
 * Compile:  gcc -g -Wall -o pth_pi_busy_wait pth_pi_busy_wait.c -lm -lpthread
 *           Needs the timer.h header file
 * Run:      pth_pi_busy_wait <number of threads> <n>
...
int main(int argc, char* argv[]) {
   ...
   printf("   Multi-threaded estimate of pi  = %.20f\n", sum);
   printf("  Tiempo Transcurrido  = %.10f seconds\n", elapsed);

  /* GET_TIME(start);
   sum = Serial_pi(n);
   GET_TIME(finish);
   elapsed = finish - start;
   printf("   Single-threaded estimate of pi = %.20f\n", sum);
   printf("   Elapsed time = %.10f seconds\n", elapsed);
   printf("   Math library estimate of pi    = %.20f\n", 
       4.0*atan(1.0));*/
